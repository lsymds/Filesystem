...
namespace Baseline.Filesystem.Tests.Adapters.S3.Integration.Files
{
    public class ReadFileAsStreamTests : BaseS3AdapterIntegrationTest
    {
        ...
        public async Task It_Retrieves_File_Contents_Stream_If_File_Does_Exist()
        {
            ...
            // Act.
            var fileContents = await FileManager.ReadAsStreamAsync(new ReadFileAsStreamRequest { FilePath = path });
            
            // Assert.
            (await new StreamReader(fileContents.FileContents).ReadToEndAsync()).Should().Be("you should check these contents");
        }
