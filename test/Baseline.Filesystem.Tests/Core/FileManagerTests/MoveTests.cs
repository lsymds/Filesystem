...
namespace Baseline.Filesystem.Tests.Core.FileManagerTests;
...
public class MoveTests : BaseManagerUsageTest
{
    ...
    public async Task It_Invokes_The_Matching_Adapters_Move_File_Method()
    {
        ...
        Adapter
            .Setup(
                x => x.MoveFileAsync(It.IsAny<MoveFileRequest>(), It.IsAny<CancellationToken>())
            )
    ...
    public async Task It_Removes_A_Root_Path_Returned_From_The_Adapter()
    {
        ...
        Adapter
            .Setup(x => x.MoveFileAsync(It.IsAny<MoveFileRequest>(), CancellationToken.None))
            .ReturnsAsync(
